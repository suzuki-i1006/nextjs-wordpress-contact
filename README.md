...
1. **メール送信の実行先を変更**
   - 変更前: Next.js API から SendGrid API を直接呼び出す
   - 変更後: Next.js API から WordPress CF7 API を呼び出す
2. **メール本文の最終管理場所を変更**
   - 変更前: Next.js 側で本文テンプレを管理
   - 変更後: CF7 側（メール / メール(2)）で本文テンプレを管理
3. **環境変数の主役を変更**
   - 変更前: `SENDGRID_API_KEY` など
   - 変更後: `WORDPRESS_API_URL`, `CF7_FORM_ID`, `WORDPRESS_API_USER`, `WORDPRESS_API_PASS` など
...
| 項目 | CF7 の `name` | Next.js 側のキー | 定義場所 |
|---|---|---|---|
| 氏名 | `your-name` | `name` | `src/components/contact-form.tsx` / `src/app/api/contact/route.ts` |
| メールアドレス | `your-email` | `email` | 同上 |
| 題名 | `your-subject` | `subject` | 同上 |
| 本文（任意） | `your-message` | `message` | 同上 |
| リンク（任意） | `url-25` | `url` | 同上 |
| 電話番号（任意） | `tel-868` | `phone` | 同上 |
| 数値（任意） | `number-833` | `numberValue` | 同上 |
| 日付（任意） | `date-537` | `dateValue` | 同上 |
| ドロップダウン（任意） | `select-946` | `selectValue` | 同上 |
| チェックボックス（任意） | `checkbox-148` | `checkboxValues` | 同上 |
| ラジオボタン | `radio-203` | `radioValue` | 同上 |
| 同意 | `acceptance-643` | `accepted` | 同上 |

> 注意: `url-25` のような自動採番の `name` はフォーム再生成で変わる場合があります。変わったら `route.ts` のマッピングも必ず更新してください。

### 移行ステップ（既存SendGridプロジェクト向け）

1. WordPress に CF7 フォームを作成し、`name` を確定する
2. `CF7_FORM_ID` を確認する
3. `src/app/api/contact/route.ts` を導入し、`buildCf7Body` を CF7 の `name` に合わせる
4. `src/components/contact-form.tsx` の入力項目と送信キーを合わせる
5. 環境変数を SendGrid から CF7 用へ置き換える
6. CF7 の「メール」「メール(2)」で本文テンプレを設定する
7. 成功 / 必須未入力 / 同意未チェック の3パターンをテストする
...
[textarea your-message placeholder "お問い合わせ内容（任意）"]
[submit "送信"]
```
